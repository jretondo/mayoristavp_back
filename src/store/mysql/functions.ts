...
export const multipleInsert = async (
      ...
      if (whereParams.mode === EModeWhere.like) {
        whereParams.items.map((item, key) => {
          if (whereParams.items.length === 1) {
            query =
              query + ` ${initWord} (${item.column} LIKE '%${item.object}%') `;
          } else {
            if (key === 0) {
              query =
                query + ` ${initWord} (${item.column} LIKE '%${item.object}%' `;
              ...
              query =
                query + ` ${concat} ${item.column} LIKE '%${item.object}%') `;
            } else {
              query =
                query + ` ${concat} ${item.column} LIKE '%${item.object}%' `;
            }
        ...
        whereParams.items.map((item, key) => {
          if (whereParams.items.length === 1) {
            ...
            if (key === 0) {
              query =
                query + ` ${initWord} (${item.column} = '${item.object}' `;
            } else if (key === whereParams.items.length - 1) {
