...
const list = (req: Request, res: Response, next: NextFunction) => {
  Controller.list(Number(req.query.idPv), Number(req.query.idProd))
    .then((lista: any) => {
      success({
        req,
        res,
...
const listaStockPDF = (req: Request, res: Response, next: NextFunction) => {
  Controller.listaStock(
    Boolean(req.query.desc),
    String(req.query.order),
    Number(req.query.prodId),
    ...
    .then((data) => {
      file(req, res, data.filePath, 'application/pdf', data.fileName, data);
    })
    .catch(next);
};
...
router.get('/listaStock/pdf', secure(EPermissions.stock), listaStockPDF);
