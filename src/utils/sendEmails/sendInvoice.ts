...
export const sendInvoice = async (
  ...
  if (importe < 0) {
    ...
    parrafosHead = [
      'En el presente email le adjuntamos el comprobante del pago.',
    ];
    ...
    asunto = 'Comprobante de compra realizada';
    informationList = [
      {
        col1: 6,
        title1: 'Nombre completo',
    ...
    parrafosHead = [
      'En el presente email le adjuntamos la comprobante de su compra.',
    ];
  ...
  return new Promise((resolve, reject) => {
    ejs.renderFile(
      path.join('views', 'emails', 'Templates', 'FactEmail.ejs'),
      datos2,
      async (err, data) => {
        if (err) {
          console.error(err);
